...
function ClassManagement(props) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: text => (
                <>
                    <span className={styles.classColor} onClick={e => {
                        updataIsFlag(true)
                        updatelist(text)
                    }}>修改</span>|
                    <span className={styles.classColor} onClick={()=>{delclass(text.grade_id)}}>删除</span>
                </>)
        },
    ];
    ...
    return (
        <Layout style={{ padding: 0 }}>
            ...
            >
                <Button type="primary" onClick={() => updateflag(true)}>+添加班级</Button>
                <Modal
                    title="添加班级"
                    visible={flag}
                    onCancel={() => { updateflag(false) }}
                    onOk={addclass}
                >
                    <Form.Item label="班级名">
                ...
                <Table columns={columns} dataSource={props.grade.getClassData} rowKey={record => `${record.grade_id}`} />
            </Content>
        </Layout>
    )
}
...
export default connect(mapStateToProps, mapDisaptchToProps)(Form.create()(ClassManagement))